...
def roll1(level): # M=1일 때 N번 주사위 던져서 나올 수 있는 모든 경우
    if level == N: # 주사위를 N번 던졌으면 결과 출력
        printpath(level)
        return
    for i in range(1, 7):
        path[level] = i # 1. 현재 레벨의 주사위 값을 i로 설정
        roll1(level+1) # 2. 다음 레벨로 재귀 사용
        path[level] = 0 # 3. 백트래킹 : 현재 레벨의 주사위 값을 초기화

def roll2(level): # M=2일 때 N번 주사위 던져서 나올 수 있는 모든 경우
    if level == N: # 주사위를 N번 던졌으면 결과 출력
        printpath(level)
        return
    for i in range(1, 7):
        if level > 0 and i <path[level-1]: # 중복 제거 조건
            continue
        path[level] = i # 1. 현재 레벨의 주사위 값을 i로 설정
        roll2(level+1) # 2. 다음 레벨로 재귀 사용
        path[level] = 0 # 3. 백트래킹 : 현재 레벨의 주사위 값을 초기화

DAT = [0] * 10

def roll3(level): # M=3일 때 N번 주사위 던져서 나올 수 있는 모든 경우
    if level == N: # 주사위를 N번 던졌으면 결과 출력
        printpath(level)
        return
    for i in range(1, 7):
        ...
        path[level] = i # 1. 현재 레벨의 주사위 값을 i로 설정
        roll3(level+1) # 2. 다음 레벨로 재귀 사용
        path[level] = 0 # 3. 백트래킹 : 현재 레벨의 주사위 값을 초기화
        DAT[i] = 0 # 백트래킹 : i 눈금 사용 체크 초기화
